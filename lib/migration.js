...
function mixinMigration(PostgreSQL) {
  ...
  PostgreSQL.prototype.showFields = function(model, cb) {
    ...
    'is_nullable AS "nullable", character_maximum_length as "length",'
  ...
  PostgreSQL.prototype.getPropertiesToModify = function(model, actualFields) {
    ...
    propNames.forEach(function(propName) {
      ...
      if (found && self.propertyHasNotBeenDeleted(model, propName)) {
        ...
        if (defaultChanged(propName, found)) {
          sql.push('ALTER COLUMN ' + self.modifyDefaultInActual(model, propName));
        }
    ...
    function defaultChanged(propName, oldSettings) {
      var newSettings = m.properties[propName];
      if (!newSettings) {
        return false;
      }
      var changed = false;
      var oldDefault = Object.prototype.hasOwnProperty.call(oldSettings, 'default') ? oldSettings.default : null;
      var newDefault = Object.prototype.hasOwnProperty.call(newSettings, 'default') ? newSettings.default : null;
  ...
  PostgreSQL.prototype.modifyDefaultInActual = function(model, propName) {
    var self = this;
    var prop = this.getPropertyDefinition(model, propName);
    var sqlCommand = self.columnEscaped(model, propName) + ' ';
    var colDefault = self.columnDbDefault(model, propName);
    if (colDefault === '') {
      sqlCommand = sqlCommand + 'DROP DEFAULT';
    } else {
      sqlCommand = sqlCommand + 'SET' + colDefault;
    }
    return sqlCommand;
  };
  ...
  PostgreSQL.prototype.buildColumnDefinitions =
    PostgreSQL.prototype.propertiesSQL = function(model) {
      ...
      Object.keys(this.getModelDefinition(model).properties).forEach(function(prop) {
        sql.push(self.addPropertyToActual(model, prop));
      });
      if (pks.length > 0) {
